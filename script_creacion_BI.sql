...
(SELECT
	...
	NULL,
	CASE
		WHEN FLOOR(DATEDIFF(DAY, C.CLIENTE_FECHA_NAC, f.FACTURA_FECHA )/ 365.2425) BETWEEN 18 AND 30 THEN 1
		WHEN FLOOR(DATEDIFF(DAY, C.CLIENTE_FECHA_NAC, f.FACTURA_FECHA )/ 365.2425) BETWEEN 31 AND 50 THEN 2
		WHEN FLOOR(DATEDIFF(DAY, C.CLIENTE_FECHA_NAC, f.FACTURA_FECHA )/ 365.2425) > 50 THEN 3
		ELSE 4
	END
FROM
	FFAN.CLIENTE C inner join FFAN.FACTURA f on c.CLIENTE_ID = f.FACTURA_CLIENTE_ID
	UNION 
	...
	NULL,
	CASE
		WHEN FLOOR(DATEDIFF(DAY, C.CLIENTE_FECHA_NAC, compra.COMPRA_FECHA )/ 365.2425) BETWEEN 18 AND 30 THEN 1
		WHEN FLOOR(DATEDIFF(DAY, C.CLIENTE_FECHA_NAC, compra.COMPRA_FECHA  )/ 365.2425) BETWEEN 31 AND 50 THEN 2
		WHEN FLOOR(DATEDIFF(DAY, C.CLIENTE_FECHA_NAC, compra.COMPRA_FECHA )/ 365.2425) > 50 THEN 3
		ELSE 4
	END
FROM
	FFAN.CLIENTE C inner join FFAN.compra compra on c.CLIENTE_ID = compra.COMPRA_CLIENTE 
	)
GO
